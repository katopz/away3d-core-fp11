package away3d.loaders.parsers
{
	...
	public class DAEParser extends ParserBase
	{
		...
		// TODO : apply bone data
		private function parseSceneGraphJoint(node:DAENode, parent:ObjectContainer3D = null, tab:String = ""):void
		{
			var _tab:String = tab + "-";

			Debug.trace(_tab + "[" + node.name + ":" + node.sid + "]");
			...
			if (node.type == "JOINT")
			{
				...
				for (var i:uint = 0; i < node.nodes.length; i++)
					parseSceneGraphJoint(node.nodes[i], container, _tab);
			}
		}
		...
		private function parseSkeletonHierarchy(node:DAENode, skin:DAESkin, skeleton:Skeleton, parent:int = -1, tab:String = ""):void
		{
			...
			else
			{
				...
				joint.inverseBindPose = node.matrix.rawData;

				skeleton.joints.fixed = false;
				skeleton.joints.push(joint);
				skeleton.joints.fixed = true;
...
class BoneData extends ObjectContainer3D
{
	/**
